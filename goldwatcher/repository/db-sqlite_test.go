...
func TestSQLiteRepository_GetHoldingByID(t *testing.T) {
	...
	if h.PurchasePrice != 1000 {
		t.Error("wrong purchase price return returned; expected 1000 but got", h.PurchasePrice)
	}

	_, err = testRepo.GetHoldingByID(2)
	if err == nil {
		t.Error("get one returned value for non-exsistent ID")
	}
}
